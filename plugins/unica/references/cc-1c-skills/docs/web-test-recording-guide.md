...
3. **Через переменную окружения** — задать `FFMPEG_PATH=C:\tools\ffmpeg\bin\ffmpeg.exe`
...
Передавай настройки голоса явно в `addNarration()` или через переменные окружения провайдера, например `OPENAI_API_KEY`.
