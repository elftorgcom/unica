...
v8-runner не управляет Apache publication, поэтому используй native script. Базу бери из `connection` в `v8project.yaml` или `V8TR_CONFIG`; если конфига нет, спроси строку соединения и предложи `/db-list init`.

**ОБЯЗАТЕЛЬНО:**
- преобразуй файловую строку `/F...` в `-InfoBasePath`;
- преобразуй серверную строку `/Sserver/ref` в `-InfoBaseServer` + `-InfoBaseRef`;
- если пользователь/пароль не указаны в connection string, спроси их при необходимости;
- путь Apache бери из `UNICA_APACHE_PATH`, явного `-ApachePath` или `tools/apache24`.

## Команда

```powershell
powershell.exe -NoProfile -File scripts/web-publish.ps1 <параметры>
```
...
4. Если проектного конфига нет — предложи `/db-list init`
...
# Серверная база
powershell.exe -NoProfile -File scripts/web-publish.ps1 -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret"

# Ручной режим (только инструкция)
powershell.exe -NoProfile -File scripts/web-publish.ps1 -InfoBasePath "C:\Bases\MyDB" -Manual
```
